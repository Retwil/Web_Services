...
/* 		prueba:function(req,res){
		...
		login:function(peticion,respuesta){			
			modelo.sequelize.query("CALL sp_autenticarUsuario('"+peticion.body.nombre+"', '"+peticion.body.contrasena+"');")
			.then(function(data){
					respuesta.json(data);
		...
		tokenGenerator:function(req,res){
			var token=jwt.sing({company:'kinal'},'S3CUR3@APP');
			res.send(token);
		},
		tokenMiddleware:function(req,res,next){
			...
			if(token) {
				...
				return res.status(403).send({
					success:false,
					message:'No se proporciono ningun token por el momento'
				});
			}
		}
	}
...
function expiresIn(dias) {
	...
	return dataObj.setDate(dateObj.getDate()+dias);
}

function genToken(user) {
	var payload=jwt.sing({
		"company":"kinal"
	},
	'S3CUR3@APP');
	var token={
